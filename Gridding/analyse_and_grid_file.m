...
%% Here are locally computed save paths - and
[~,saved_filename,~] = fileparts(file_path);

OPTS.save_GEO = fullfile(OPTS.processed_data_subdir,'GEO',saved_filename);

for proc_ind = 1:length(PROCESSES)

    OPTS.save_loc{proc_ind} = fullfile(OPTS.processed_data_subdir,PROCESSES(proc_ind).name,saved_filename);
