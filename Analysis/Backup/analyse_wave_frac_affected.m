...
%     ssh_flag = ssh_flag(b); 
    ...
    if isempty(dist(isocean))
        
        dist_to_ssh = nan*dist;
        
    else
        
        [~,dist_to_ssh] = knnsearch(dist(isocean),dist);
    end
    
    if sum(isocean) > 1
        
        % This is the interpolated SSH field, on all of the dist places 
        height_interp = interp1(dist(isocean),height(isocean),dist);
        
        
    else
        
        height_interp = nan*dist;
    ...
    % SSH 
    ssh = cat(1,ssh,height_interp); % movmean(height_interp,window_50k,'samplepoints',dist));
    ...
    moving_ssh_no = cat(1,moving_ssh_no,movsum(isocean,window_10k,'samplepoints',dist));
    
    ssh_moving_std = cat(1,ssh_moving_std,movstd(height_interp,window_10k,'samplepoints',dist));
