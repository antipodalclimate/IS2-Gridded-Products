...
for gran_ind = 1:length(GEO_files)
    ...
    STATS.beamflag(isnan(STATS.beamflag)) = 0; 
    ...
    try
    TEMP.ice_length_weak{gran_ind} = reshape(bsxfun(@times,LIFDATA.ice_length_n,(~STATS.beamflag)'),[size(GEODATA.lat) n_gran]);
    catch err
    ...
    TEMP.specular_length_weak{gran_ind} = reshape(bsxfun(@times,LIFDATA.specular_length_n,(~STATS.beamflag)'),[size(GEODATA.lat) n_gran]);
    ...
    TEMP.water_length_weak{gran_ind} = reshape(bsxfun(@times,LIFDATA.water_length_n,(~STATS.beamflag)'),[size(GEODATA.lat) n_gran]);
...
ice_length_weak = cat(3,TEMP.ice_length_weak{:});
water_length_weak = cat(3,TEMP.water_length_weak{:});
spec_length_weak = cat(3,TEMP.specular_length_weak{:});
...
OUT.LIF.LIF_weak(:,:,mo_ind,yr_ind) = sum(ice_length_weak,3)./(sum(water_length_weak,3) + sum(ice_length_weak,3));
...
OUT.LIF.LIF_spec_weak(:,:,mo_ind,yr_ind) = (sum(ice_length_weak,3) + sum(spec_length_weak,3))./(sum(water_length_weak,3) + sum(ice_length_weak,3));
...
OUT.LIF.SIC_SSMI_weak(:,:,mo_ind,yr_ind) = mean(TEMP.SIC_SSMI(:,:,~TEMP.beamstr),3,'omitmissing');
...
OUT.LIF.SIC_AMSR_weak(:,:,mo_ind,yr_ind) = mean(TEMP.SIC_AMSR(:,:,~TEMP.beamstr),3,'omitmissing'); 